...
{		
	...
	long firstVehicleID;
	long lastVehicleID;
	float time, timeHeadway, lastTimeHeadway, LeftTime, oldLeftTime;
	float randNumber, tmp, tmp1, tmp2;
	float ACC_proportion;
	float lastEntryTime;
	...
	// Implement tests in which the proportion of ACC vehicles
	for(j=0;j<40;j++)
	{
		ACC_proportion=j*0.025;
		firstVehicleID=1;
		lastEntryTime=0;
		time=0.0;
		...
		tmp2=sqrt(fabs(tmp1));
		for(i=0;i<=numberOfVehicles-1;i++) vehiclePlatoon.erase(vehiclePlatoon.begin());

		printf("%4d\n", j);
		fprintf(fd2,"%4.2f\t", j*0.025);
		fprintf(fd2,"%8.4f\t", tmp);
		fprintf(fd2,"%8.4f\t", tmp1);
		fprintf(fd2,"%8.4f\t", tmp2);
		fprintf(fd2,"%8.4f\n", tmp2/tmp);
	}//end of for
...
vehicle gipps(int counter, float time, vehicle veh1, vehicle veh2)
{
	...
	float 	lambda = 0.3;
	float	v_desired = 40 * 1.6* 1000 /(3600);
	float	accl_max=1.7;
...
float inFlow(float time)
{
	//if(time>=100 && time<200) return time*0.01;
	//else 
	return 0.5;
}
